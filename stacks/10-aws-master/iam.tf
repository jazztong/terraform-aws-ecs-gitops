     ...
     "Condition": {
       ...
       "StringLike": {
         "${var.tfc_hostname}:sub": [
            "organization:${var.tfc_organization_name}:project:AWS-Project:workspace:aws-preprod:run_phase:*",
            "organization:${var.tfc_organization_name}:project:AWS-Project:workspace:aws-prod:run_phase:*"
         ]
       }
     }
   }
 ...
 "Statement": [
   {
     ...
     "Resource": [
       ...
       "arn:aws:iam::${aws_organizations_account.preprod.id}:role/OrganizationAccountAccessRole"
     ]
   }
 ]
}
